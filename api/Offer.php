...
class Offer extends Simpla
{
    ...
    */
    public function get_posts($filter = array())
    {
        ...
        $query = $this->db->placehold("SELECT b.id, b.url, b.name, b.annotation, b.text,
		                                      ...
		                                      b.date
		                                      FROM __offer b WHERE 1 $post_id_filter $visible_filter $keyword_filter
    ...
    */
    public function add_post($post)
    {
        if(!isset($post->date))
            ...
            $date_query = '';
        $query = $this->db->placehold("INSERT INTO __offer SET ?% $date_query", $post);
    ...
    */
    ...
    {
        $query = $this->db->placehold("UPDATE __offer SET ?% WHERE id in(?@) LIMIT ?", $post, (array)$id, count((array)$id));
        $this->db->query($query);
        return $id;
    }
    ...
    */
    public function delete_post($id)
    {
        if(!empty($id))
        {
            $query = $this->db->placehold("DELETE FROM __offer WHERE id=? LIMIT 1", intval($id));
            if($this->db->query($query))
            {
                $query = $this->db->placehold("DELETE FROM __comments WHERE type='offer' AND object_id=?", intval($id));
                if($this->db->query($query))
                    return true;
            }
        }
    ...
    */
    ...
    {
        $this->db->query("SELECT date FROM __offer WHERE id=? LIMIT 1", $id);
        $date = $this->db->result('date');

        $this->db->query("(SELECT id FROM __offer WHERE date=? AND id>? AND visible  ORDER BY id limit 1)
		                   UNION
		                  (SELECT id FROM __offer WHERE date>? AND visible ORDER BY date, id limit 1)",
            $date, $id, $date);
    ...
    */
    public function get_prev_post($id)
    {
        ...
        $this->db->query("(SELECT id FROM __offer WHERE date=? AND id<? AND visible ORDER BY id DESC limit 1)
		                   UNION
		                  (SELECT id FROM __offer WHERE date<? AND visible ORDER BY date DESC, id DESC limit 1)",
            $date, $id, $date);

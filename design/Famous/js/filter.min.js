...
$(document).ready(function(){
    ...
    $('#variants_width').change(ajaxRecount);
    $('#variants_height').change(ajaxRecount);
    $('#variants_color').change(ajaxRecount);

	$('#filter form input[type="checkbox"]').on('change', ajaxRecount);
	$('#slider_price').on('slider_price_change', ajaxRecount );
	$('form').on('click','.apply', function(){
			...
			//featured
				...
				$.each(responseText.features, function(i, v) {
					$.each(v.options, function(k, o) {
						...
						selector.parent().find('.count').text('(' + o.count + ')');
				...
				$this.parents('form').find('.modef').remove();
...
function popover(z){
	return "<div id='modef' class='modef'><i class='fa fa-search'></i> <span id='modef_num'>" + 'Найдено ' +z + "</span>&nbsp;<a class='showchild littlepopup_link apply' href='#'>Показать</a></div>";
}

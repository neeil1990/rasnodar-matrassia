...
//addSku
$(document).on("click", ".addSku", function(event) {
	var $this = $(this);
	if ($this.data("id") != "" && $this.attr("href") == "#") {
		$this.addClass("loading").text("Caa?ocea");
		$.getJSON(ajaxPath + "?act=skuADD&id=" + $this.data("id") + "&ibl=" + $this.data("ibl"), function(data) {
			if (data != "") {

				$this.text("A ei?ceio").removeClass("loading");
				...
				$.each(data, function(index, first){
					...
					$.each(first["PROPERTIES"], function(i, prop){
						if(prop["SORT"] <= 100 && i !== "CML2_LINK"){
							propStr += '<tr><td>' + prop ["NAME"] + '</td><td>' + prop ["VALUE"] + '</td></tr>';
						}
					});

					var quantityStr = first["CATALOG_QUANTITY"] > 0 ? '<span class="available">' + LANG["AVAILABLE"] + '</span></li>' : '<span class="noAvailable">' + LANG["NOAVAILABLE"] + '</span></li>';

					$_self.append('<li>' +
						'<div class="skuItem">' +
							'<a href="' + first["DETAIL_PAGE_URL"] +'" class="name">' + first["NAME"] +'</a>' +
							'<div class="wrap">' +
							'<a href="' + first["DETAIL_PAGE_URL"] +'" class="pic"><img src="' + first["DETAIL_PICTURE"] +'" alt=""></a>' +
							'<ul>' +
								'<li>' +
									'<span class="price">' + first["PRICE"] +	'</span>' +
								'</li>' +
								'<li>' + quantityStr + '</li>' +
								'<li><a href="#" data-id="' + first["ID"] + '" class="addCart btn' + (first["ADDCART"] == false ? " disabled" : "") + '"' + ($this.data("reload") === "Y" ? ' data-reload="Y"' : '') + ' data-mobile="Y">' + LANG["ADD_CART"] + '</a></li>' +
							'</ul>' +
							'</div>' +
							'<table class="skuParams">' + propStr + '</table>' +
						'</div>' +
					'</li>');
				});
				...
				$("html").addClass("bx-menu").hasClass("bx-transform") ? $_main.addClass("opened") : $_main.animate({left: "0"}, 100);

				return elOpen["sku"] = true;
...
$(document).on("click", ".addCompare", function(e) {
	...
	if ($this.data("id") != "" && $this.attr("href") == "#") {
		$.getJSON(ajaxPath + "?act=compADD&id=" + $(this).data("id"), function(data) {
			if (data != "") {
				$img = $this.find("img");
				$this.html(" A nienea n?aaiaiey").prepend($img).addClass("added").attr("href", SITE_DIR + "compare/");
			...
			if($addElements.length > 0){
				...
				if(_arID != ""){
					$.getJSON(ajaxPath + "?act=add&id=" + _arID.join(";") + "&q=1&multi=1", function(data) {
						if (data != "") {
							$this.text("A ei?ceia").attr("href", SITE_DIR + "personal/cart/").removeClass("loading").addClass("added");
							cartReload();
						}else{
							alert("error(3)");
						}
		...
		if ($this.data("id") != "" && $this.attr("href") == "#") {
			if(!$this.hasClass("disabled")){
				...
				$.getJSON(ajaxPath + "?act=add&id=" + $this.data("id") + "&q=1", function(data) {
					if (data != "") {
						$this.text("Ioi?ieou caeac").attr("href", SITE_DIR + "personal/cart/").removeClass("loading").addClass("added");
...
$(document).on("click", "#inBasketDelete", function(e) {
	...
	$.get(ajaxPath + "?act=del&id=" + obj.data("id"), function(data) {
		if (data != "") {
			...
			}).removeClass("added").text("A ei?ceio").attr("href", "#");
			cartReload();
		} else {
...
var cartReload = function() {
	$.get(ajaxPath + "?act=flushCart", function(data) {
		...
		$("#footerCart").html($items.eq(1).find("#footerCart").html());
	});
}
...
$(window).on("load",  {flag: false}, initialization);
$(window).on("resize", {flag: true}, initialization);

$(document).ready(function(event) {

	$(".showMore").on("click", function(event) {
		$(this).hide().siblings(".productList").find("li").show();
		return event.preventDefault();
	});

	var closeSensitivityX = 45;
	var closeSensitivityY = 15;
	...
	var closeSlideBlock = function(event){

		if(elOpen["menu"]){
			removeTools($navContainer, "menu");
		}
		else if(elOpen["sku"]){
			removeTools($inSku, "sku");
		}
		else if(elOpen["filter"]){
			removeTools($smFilter, "filter");
		}
	};

	var startTouchFN = function(event){
		startTouchX = event.originalEvent.touches[0].pageX;
		startTouchY = event.originalEvent.touches[0].pageY;
	};

	var touchmove = function(event){
		if(elOpen["menu"] == true || elOpen["sku"] == true || elOpen["filter"] == true){
			var movedX = startTouchX - event.originalEvent.touches[0].pageX;
			var movedY = Math.abs(startTouchY - event.originalEvent.touches[0].pageY);
			if(movedX > closeSensitivityX && movedY < closeSensitivityY && elOpen["filter"] != true || movedX < -closeSensitivityX && movedY < closeSensitivityY && elOpen["filter"] == true){
				if(elOpen["menu"]){
					removeTools($navContainer, "menu");
				}
				else if(elOpen["sku"]){
	...
	var removeTools = function($element, rLink){
		$backdrop.fadeOut();
		$layout.removeClass("opened");
		$html.removeClass("bx-menu").hasClass("bx-transform") ? $element.removeClass("opened") : $element.animate({left: "-260px"}, 100);
		if(rLink == "filter"){
			$swClose.removeClass("sm");
		}
	
		$(window).scrollTop(rScroll);
		return elOpen[rLink] = false;
	
	};

	$(document).on("click", "#backdrop", closeSlideBlock);
	$(document).on("touchstart", startTouchFN);
...
(function($) {
    jQuery.fn.electroSlider = function(options) {
        ...
        var link = this;
        var slideBox = link.find("ul").css("left", "0px");
        ...
        if(options["touch"]){
            ...
            slideBox.on("mousedown touchstart", function(e){
                ...
                slideStartPosition = parseInt(slideBox.css("left"), 10); console.log(slideBox.css("left"));
            ...
            $(document).on("mousemove touchmove", function(e) {
                if(clickControl == true){
                    ...
                    if(Math.abs(clickPositionY - curTouchPosY) > 50){
                        e.preventDefault();
                    }else{
            ...
            $(document).on("mouseup touchend", function(e) {
                if(clickControl == true){
                    ...
                    var boxPosition = parseInt(slideBox.css("left"));
...
$(document).ready(function() {
	...
	$("#csTrigger").on("click", function(e) {
		colorSwitcher.toggleClass("opened");
		e.preventDefault();
	});
	...
	$('.colorTrigger').on('click', function(e) {
		...
		switchTheme(colorScheme, 'mobile');
		e.preventDefault();
	});

	function switchTheme (colorScheme) {
		if (!navigator.cookieEnabled) {
			alert("?�???�NZN�??N�?� cookie ???�N? ??N�??N?????N�N�?� ??N�N?????N� N�???�N�????N�N� N?N�?�??");
		} else {
	...
	var changeSearch = function(event){
		...
		if($this.val() != "" && $this.val().length > 1){
			...
			searchTimeout = setTimeout(function(){
				sendDataSearch($this.val(), 0, $this.val(), $this.data("id"), $("#searchForm").attr("action"));
			}, 250);
		}else{
			clearTimeout(searchTimeout);
			makeObject(false);
			...
			for (var i = 0; i <= data.length -1; i++){
				$mainTable.append(
					$("<tr/>").append(
						...
						'<td class="bask"><a href="#" class="' + (data[i]["SKU"] == true ? "addSku" : "addCart") + (data[i]["ADDCART"] == false ? " disabled" : "") + '" data-id="' + data[i]["ID"] + '" data-ibl=' + data[i]["IBLOCK_ID"] + ' data-mobile="Y">' + (data[i]["SKU"] == true ? LANG["ADD_SKU"] : LANG["ADD_CART"]) + '</a></td>'
					)
				);
			}
		}else{
...
$(document).on("ready", function() {
	...
	var openMenu = function(event){
		...
		$html.addClass("bx-menu").hasClass("bx-transform") ? $navContainer.addClass("opened") : $navContainer.animate({left: "0"}, 100);

		return event.preventDefault(elOpen["menu"] = true);
	};

	$(document).on("click", "#menuTrigger", openMenu);

    $('.categories_link.selected').parents('.categories_list_collapsed').slideToggle(0);
    $('.categories_link.selected').parents('.categories_list').find('.categories_link_parent_btn').toggleClass('rotated');

    $('.categories_link_parent_btn').on('click',function(e){
        $(this).siblings('.categories_list_collapsed').slideToggle(300);
        $('.categories_link.selected').parents('.categories_list_collapsed').find('.categories_link_parent_btn').toggleClass('rotated');

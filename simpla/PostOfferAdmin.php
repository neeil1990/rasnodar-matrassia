...
class PostOfferAdmin extends Simpla
{
    public function fetch()
    {
        ...
        if($this->request->method('post'))
        {
            ...
            if(($a = $this->offer->get_post($post->url)) && $a->id!=$post->id)
            {
            ...
            else
            {
                if(empty($post->id))
                {
                    $post->id = $this->offer->add_post($post);
                    $post = $this->offer->get_post($post->id);
                    $this->design->assign('message_success', 'added');
                }
                else
                {
                    $this->offer->update_post($post->id, $post);
                    $post = $this->offer->get_post($post->id);
        ...
        else
        {
            ...
            $post = $this->offer->get_post(intval($post->id));
        }
        ...
        return $this->design->fetch('post_offer.tpl');
    }
}
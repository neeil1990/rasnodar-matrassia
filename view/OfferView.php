 ...
 *
 * Этот класс использует шаблоны offer.tpl и post.tpl
 *
 */
...
class OfferView extends View
{
    ...
    private function fetch_post($url)
    {
        ...
        // Комментарии к посту
        $comments = $this->comments->get_comments(array('type'=>'offer', 'object_id'=>$post->id, 'approved'=>1, 'ip'=>$_SERVER['REMOTE_ADDR']));
        $this->design->assign('comments', $comments);
        $this->design->assign('post',      $post);

        // Соседние записи
        $this->design->assign('next_post', $this->offer->get_next_post($post->id));
        $this->design->assign('prev_post', $this->offer->get_prev_post($post->id));
    ...
    private function fetch_offer()
    {
        ...
        $body = $this->design->fetch('offer.tpl');
